...
def get_pie(entered_site):
    ...
    if entered_site=='ALL':
        #df=df[df['Launch Site']==str('entered_site')]
        fig=px.pie(df, 
                   values='class', 
                   names='Launch Site',
                   title='Total Success by Site')
        
        #fig = px.pie(values=[0, 1, 2, 3, 4], names=[0, 1, 4, 9, 16])
        return fig
    else:
        ...
        #fig = px.pie(values=[0, 1, 2, 3, 4], names=[0, 1, 4, 9, 99])
        return fig    
...
def get_scatter(entered_site, entered_payload):
    ...
    df=df[df['Payload Mass (kg)']<=max(entered_payload)]
    df=df[df['Payload Mass (kg)']>=min(entered_payload)]
    #df=df[df['Payload Mass (kg)<=']]
    if entered_site=='ALL':
